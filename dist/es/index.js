...
export const useWindowScroller = (initialWidth = 1280, initialHeight = 720, options = emptyObj) => {
  var _options$scroll;

  const scrollY = useWindowScroll(((_options$scroll = options.scroll) === null || _options$scroll === void 0 ? void 0 : _options$scroll.fps) || 8);

  const _ref_0 = useState(false);

  const setIsScrolling = _ref_0[1];
  const isScrolling = _ref_0[0];
  const [width, height] = useWindowSize(initialWidth, initialHeight, options.size || defaultSizeOpt);
...
const getContainerRect = memoizeOne((element, width, top) => [{
  width,
  top
...
const assignUserStyle = memoizeOne((containerStyle, userStyle) => Object.assign({}, containerStyle, userStyle), (args, pargs) => args[0] === pargs[0] && args[1] === pargs[1]);
const assignUserItemStyle = trieMemoize([WeakMap, OneKeyMap], (itemStyle, userStyle) => Object.assign({}, itemStyle, userStyle));

const defaultGetItemKey = (_, i) => i; // the below memoizations for for ensuring shallow equal is reliable for pure
// component children


const getCachedSize = memoizeOne(width => ({
  width,
  zIndex: -1000,
  visibility: 'hidden',
  position: 'absolute',
...
const getCachedItemStyle = trieMemoize([OneKeyMap, Map, Map], (width, left, top) => ({
  top,
  left,
  width,
  writingMode: 'horizontal-tb',
...
const getRefSetter = trieMemoize([OneKeyMap, OneKeyMap, OneKeyMap], (resizeObserver, positionCache, itemPositioner) => trieMemoize([{}], index => el => {
  if (el === null) return;
...
export const FreeMasonry = React.forwardRef(({
  ...
  const resizeObserver = useMemo(() => new ResizeObserver(_ref4), [itemPositioner, positionCache]); // cleans up the resize observers when they change or the
  // component unmounts
...
FreeMasonry.propTypes = {
  ...
  containerRef: _pt.oneOfType([_pt.func, _pt.any])
};
export const Masonry = React.memo(React.forwardRef((props, ref) => {
  const {
    width,
    height,
    scrollY,
